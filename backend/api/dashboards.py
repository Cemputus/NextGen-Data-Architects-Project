...
# End-user messages when GET /api/dashboards/current returns no dashboard
...
def swap_dashboard():
  ...
  try:
    ...
    with engine.connect() as conn:
      ...
      row = conn.execute(
        text("SELECT id FROM dashboards WHERE id = :id AND is_active IS TRUE"),
        {"id": dashboard_id},
      ).scalar()
      if not row:
...
@dashboard_manager_bp.route("/reset-to-default", methods=["POST"])
@jwt_required()
def reset_role_to_default_dashboard():
  """
  Point a role's current dashboard at the system default template (same as first-time seed).
  Creates the default dashboard row if missing. Analyst/sysadmin only.
  Body: { "role": "<role_name>" }.
  """
  username, role = _current_user()
  if not _is_analyst_or_admin(role):
  ...
  if role == "analyst" and target_role == "sysadmin":
    return jsonify({"error": "Analysts cannot reset the Admin role's dashboard."}), 403
    ...
    with engine.connect() as conn:
      ...
      row = conn.execute(
        text(
          ...
          WHERE is_active = TRUE AND name = 'Default Analytics Dashboard'
      ...
      if row:
        ...
        conn.execute(
          ...
          ),
          {
            "id": default_id,
            "name": "Default Analytics Dashboard",
            "description": "System default layout (KPIs + core charts).",
            "created_by_username": username or "system",
            "created_by_role": role or "sysadmin",
            "definition": json.dumps(
              {
                "template": "analytics_dashboard",
      ...
      conn.commit()
    return jsonify({"ok": True, "dashboard_id": default_id}), 200
  except Exception as e:
    if engine is not None:
      engine.dispose()
    return jsonify({"error": str(e)}), 500
  ...
  try:
    ...
    with engine.connect() as conn:
      ...
      dash = {
        ...
        "created_by_username": row["d_created_by_username"],
        "created_by_role": row["d_created_by_role"],
        "definition": row["d_definition"],
        "updated_at": row["d_updated_at"],
        "roles": [rr for rr in role_rows],
        "users": [uu for uu in user_rows],
      }
