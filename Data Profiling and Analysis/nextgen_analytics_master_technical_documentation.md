...
- school recruitment and high school analytics (feeder schools, recruitment insights, marketing dashboards)
...
## 15) School Recruitment and High School Analytics

This section merges school-related recruitment and analytics documentation into the master reference. High school data links each student to their **previous high school and district**, enabling feeder analysis, recruitment strategy, and marketing intelligence.

### 15.1 High School Analytics Overview

The dataset supports:

- **Feeder schools** — which schools send the most students
- **Recruitment regions** — geographic distribution of intake
- **Academic preparedness** — performance by source school
- **Retention patterns** — persistence by high school
- **Graduation outcomes** — completion and CGPA by school
- **Financial behavior** — payment and sponsorship by recruitment pipeline

Use cases:

- recruitment strategy and targeting
- marketing and partnership programs
- regional expansion planning

### 15.2 Dataset Structure (High School Context)

**High school dimension (logical):**

- **Concept:** `high_schools_dimension` (or equivalent warehouse view / `dim_student` attributes).
- **Columns (logical):** `SCHOOL_NAME`, `DISTRICT`, `SCHOOL_TIER`, `OWNERSHIP`.
- **Purpose:** Dimension table for school-level analytics.

**Student–high school linkage:**

- **Sources / views:** `student_high_schools_list15`, `student_high_schools_list16`, `student_high_schools_all` (or derived from `students_list15/16` / warehouse `dim_student` with `high_school`, `high_school_district`).
- **Columns (logical):** `REG_NO`, `ACC_NO`, `PROGRAM`, `HIGH_SCHOOL`, `DISTRICT`, `SCHOOL_TIER`, `OWNERSHIP`, `STUDENT_LIST`.
- **Grain:** one row per student.

**Warehouse integration:** The primary join key is `REG_NO`. High school data joins to students, `student_grades`, `student_transcript`, `fact_student_academic_performance`, `student_attendance`, `student_payments`, and `student_sponsorships`.

### 15.3 School Recruitment Insights (SQL Examples)
...
*Strategic value:* Focus recruitment visits on these schools.
...
*Strategic value:* Target districts with high potential.
...
JOIN student_transcript t ON s.REG_NO = t.REG_NO
GROUP BY s.HIGH_SCHOOL;
```

*Insight:* Identify schools that produce stronger academic performers.
...
FROM student_high_schools_all s
JOIN student_grades g ON s.REG_NO = g.REG_NO
GROUP BY s.HIGH_SCHOOL;
```

*Insight:* Identify schools whose graduates struggle academically.
...
FROM student_high_schools_all s
JOIN student_attendance a ON s.REG_NO = a.REG_NO
GROUP BY s.HIGH_SCHOOL;
```

*Insight:* Measure engagement levels by feeder school.

**Scholarship distribution:**

```sql
SELECT s.HIGH_SCHOOL, COUNT(sp.SPONSOR_PAYMENT_ID)
FROM student_high_schools_all s
JOIN student_sponsorships sp ON s.REG_NO = sp.REG_NO
GROUP BY s.HIGH_SCHOOL;
```

*Insight:* Identify schools that produce more scholarship recipients.

### 15.4 Marketing and Recruitment Dashboard

**Purpose:** Provide admissions teams with data-driven recruitment intelligence by integrating students, high school data, transcripts, payments, attendance, and sponsorships.
...
**Dashboard sections (query patterns):**

- **Recruitment source analysis:** Top feeder schools — `SELECT HIGH_SCHOOL, COUNT(REG_NO) FROM student_high_schools_all GROUP BY HIGH_SCHOOL ORDER BY COUNT(REG_NO) DESC`.
- **Geographic recruitment map:** Students by district — `SELECT DISTRICT, COUNT(*) FROM student_high_schools_all GROUP BY DISTRICT`.
- **Academic performance by school:** Join to transcript and average `SEMESTER_GPA` by `HIGH_SCHOOL`.
- **Program preference by school:** `SELECT HIGH_SCHOOL, PROGRAM, COUNT(*) FROM student_high_schools_all GROUP BY HIGH_SCHOOL, PROGRAM`.
- **Financial stability by school:** Join to payments and average `AMOUNT_UGX` by `HIGH_SCHOOL`.
- **Retention analysis:** Join to transcript and compute share with `CGPA >= 2` by `HIGH_SCHOOL`.

### 15.5 Advanced Analytics Opportunities

- **Recruitment efficiency score:** Combine GPA, attendance, and retention to rank feeder schools for targeting and partnerships.

### 15.6 Strategic Value

High school analytics allows the institution to:
...
The integration of high school recruitment data with academic and financial data supports a **complete recruitment intelligence system** for admissions strategy, marketing campaigns, academic planning, and institutional growth.
